...
const info = async () => {
  ...
  questions.map((item) => {
    let data = ` <div data-column="question" data-order="desc" id="displayData" key = ${
      item.id
    } >
       <p class="fw-bold"> id: <span class="text-dark"> ${item.id} </span> </p>
       <p class= "fw-bold"> Question: <span > ${item.question} </span></p>
       <p class= "fw-bold choices "> Choices: ${item.choices.map((val) =>
         val ? `<span class="text-primary" > ${val.value} </span>` : "Not Found"
       )}  <span > </span></p >
       <p class= "fw-bold"> Answer: <span > ${item.answer} </span></p>
       <p class= "fw-bold text-danger d-block"> Time Delay: <span class="text-danger time d-inline"> ${
         item.timeDelay
       } </span></p>
       <button class="btn btn-danger btn-sm" onclick="deleteEl(${
         item.id
       })"> Delete </button>
       </div> `;
